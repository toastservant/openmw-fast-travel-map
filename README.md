...
- `mygui/openmw_map_window.layout` – overrides the map layout to draw the overlay inside the map so it zooms/pans with it. The `FastTravelRoutes` widget uses `position_real="0 0 1 0.88"` with `align="HStretch Top"` so the height is slightly shorter than the map. If the overlay seems vertically stretched/squashed, tweak the 4th number (height) and, if needed, add a small y offset in the 2nd number (e.g., `0.02`) to align top/bottom.
